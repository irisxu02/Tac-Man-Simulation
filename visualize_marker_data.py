...
def visualize_contact_deviations(marker_data, save_path=None, view='xz', magnification=10.0):
    ...
    for i in range(1, len(marker_data)-1):
        if marker_data[i-1]['state'] == 'PROC' and marker_data[i]['state'] == 'RECV' and marker_data[i+1]['state'] == 'PROC':
    ...
    for triple_num, (idx_proc1, idx_recv, idx_proc2) in enumerate(sequences):
        ...
        for j, idx in enumerate([idx_proc1, idx_recv, idx_proc2]):
            ...
            ax = axs[j]
            if len(unlocked) > 0:
                ax.scatter(unlocked[:, ax_ix[0]], unlocked[:, ax_ix[1]],
                           c='lightgray', s=15, alpha=0.3, zorder=1)
            ax.scatter(init_pos[:, ax_ix[0]], init_pos[:, ax_ix[1]],
                       c='black', s=20, alpha=0.5, zorder=2)
            q = ax.quiver(
                init_pos[:, ax_ix[0]], init_pos[:, ax_ix[1]], U, V,
                color='tab:blue',
                angles='xy', scale_units='xy', scale=1/(magnification),
                width=0.003, headwidth=4, headlength=5, zorder=3
            )
            ref_len = 0.001
            # Only show the scale unit on the first subplot
            if j == 0:
                ax.quiverkey(q, 0.9, 1.02, ref_len, f'{ref_len*1000:.0f} mm scale',
...
if __name__ == "__main__":
    path = "marker_data/44853_trial_4.pt"
    data = torch.load(path)
    visualize_contact_deviations(data, save_path="deviations.png", view='xz', magnification=5.0)